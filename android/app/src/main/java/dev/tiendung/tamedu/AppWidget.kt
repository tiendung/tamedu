        ...
        when (intent.action) {
            ...
            SPEAK_REMINDER_TOGGLE -> {
                if (tamedu.phap.isPlaying()) {
                    txt = if (tamedu.phap.isThuGian()) tamedu.phap.finishPhap() else tamedu.phap.pausePhap(context)
                } else {
            ...
            NGHE_VQDD -> txt = tamedu.phap.ngheVqdd(context)
            THU_GIAN -> txt = tamedu.phap.startPlayThuGian(context)
            THU_NOI -> txt = tamedu.phap.ngheThuNoi(context)
...
internal fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
    ...
    if (!tamedu.phap.isPause()) { updateViews(context, views, APP_TITLE) }
